...
class InputReaderInterface(object):
    ...
    def _limit_thrust(self, thrust, althold, emergency_stop):
        ...
        if abs(limited_thrust) > self.input.max_thrust:
            limited_thrust = (limited_thrust/abs(limited_thrust))*self.input.max_thrust
        if abs(limited_thrust) < self.input.min_thrust:
            limited_thrust = 0
